...
function CT:buildUptimeGraph()
  ...
  do
    ...
    graph:AddDataSeries(self.graphData, (self.graphColor or {0.25, 0.25, 1.0, 1.0}))
...
-- Resource Graphs
--------------------------------------------------------------------------------
function CT:resourceGraphUpdate(spell, time)
  if not self.graphData then
    self.graphData = {}
  end
...
local function normalGraphOnUpdate(self, mouseOver, dot, UIScale)
  ...
  if line then
    ...
    local dataX = self.graphData[lineNum + 1] or self.graphData[lineNum]
    local dataY = self.graphData[lineNum + 1] or self.graphData[lineNum]
    dataX = dataX[1]
    dataY = dataY[2]
